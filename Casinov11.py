...
user_bonus_claimed = set()  # Track users who claimed bonus
...
class Game:
    def __init__(self, user_id, username, bet_amount, rounds, throw_count, game_type):
        ...
        self.bot_results = []
        self.is_demo = False
        self.bot_first = False  # Track if bot rolls first
...
async def hb_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or len(context.args) == 0:
        await update.message.reply_html(
            f"🏦 <b>House Balance</b>\n\n"
            f"💎 Total: <b>{house_balance:.2f} TON</b>\n\n"
            f"To change house balance, use:\n"
            f"/hb <amount>\n"
            f"Example: /hb 5000"
        )
        return
    ...
    try:
        ...
        if new_balance < 0:
            await update.message.reply_html("❌ House balance cannot be negative!")
            return
        
        global house_balance
        old_balance = house_balance
        house_balance = new_balance
        
        await update.message.reply_html(
            f"✅ <b>House balance updated!</b>\n\n"
            f"💎 Old balance: <b>{old_balance:.2f} TON</b>\n"
            f"💎 New balance: <b>{house_balance:.2f} TON</b>"
        )
        
        logger.info(f"Admin {user_id} changed house balance: {old_balance} -> {house_balance}")
        
    except ValueError:
        await update.message.reply_html("❌ Invalid amount! Please enter a valid number.")
    except Exception as e:
        logger.error(f"House balance error: {e}")
        await update.message.reply_html("❌ An error occurred while updating house balance.")

async def bonus_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Give user 20 stars bonus (one-time)"""
    user_id = update.effective_user.id
    
    if user_id in user_bonus_claimed:
        await update.message.reply_html(
            "❌ <b>You have already claimed your bonus!</b>\n\n"
            "The bonus can only be claimed once per user."
        )
        return
...
async def withdraw_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.chat.type != 'private':
        await update.message.reply_html(
            "⚠️ <b>This command is only accessible in DM (private chat).</b>\n\n"
            "Please send me a private message to use /withdraw"
        )
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "show_withdraw":
            # Check if bot can send DM to user
            try:
                welcome_text = (
                    "✨ <b>Welcome to Stars Withdrawal !</b>\n\n"
                    "<b>Withdraw:</b>\n"
                ...
                await query.edit_message_text(
                    "✅ <b>Withdrawal information sent to your DM!</b>\n\n"
                ...
                await query.edit_message_text(
                    "❌ <b>Cannot send DM!</b>\n\n"
                    "Please start a conversation with the bot first, then use /withdraw in DM.",
                    parse_mode=ParseMode.HTML
                )
            return
        ...
        if data.startswith("throws_"):
            ...
            # Show option for bot to roll first
        ...
        # Handle bot first/user first selection
...
async def handle_game_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(game.user_results) % game.throw_count == 0:
        ...
        # Only make bot rolls if bot hasn't rolled yet for this round
        current_round_index = (len(game.user_results) // game.throw_count) - 1
        expected_bot_results = (current_round_index + 1) * game.throw_count
        
        # Bot rolls only if it hasn't rolled enough for this round
        if len(game.bot_results) < expected_bot_results:
            bot_results = []
...
def main():
    ...
    application.add_handler(CommandHandler("bal", bal_command))  # Alias
    application.add_handler(CommandHandler("deposit", deposit_command))
    application.add_handler(CommandHandler("depo", depo_command))  # Alias
