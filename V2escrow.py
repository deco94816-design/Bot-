...
async def delete_user_command(update: Update):
    ...
    try:
        # Only delete if message is from a user (not from bot)
...
async def done_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if user is bot admin
    if not is_bot_admin(user_id):
        msg = await update.message.reply_text("❌ Only bot admins can complete deals!")
        context.job_queue.run_once(lambda ctx: msg.delete(), 5)
        return
    
    # Check if replying to a confirmed deal message
    if update.message.reply_to_message:
        replied_text = update.message.reply_to_message.text
        
        # Extract Trade ID from the replied message
        trade_id_match = re.search(r'Trade ID: (#TID\w+)', replied_text)
        
        if trade_id_match:
            trade_id = trade_id_match.group(1)
            
            # Check if this trade exists in confirmed deals
            if trade_id in confirmed_deals:
...
🛡️ Escrowed By: {deal['escrow_admin']}
                ...
                msg = await update.message.reply_text("❌ Trade ID not found or deal already completed.")
                context.job_queue.run_once(lambda ctx: msg.delete(), 5)
        else:
            msg = await update.message.reply_text("❌ Could not find Trade ID in the replied message. Please reply to a confirmed deal message.")
            context.job_queue.run_once(lambda ctx: msg.delete(), 5)
    else:
        msg = await update.message.reply_text("❌ Please reply to a confirmed deal message with /done")
        context.job_queue.run_once(lambda ctx: msg.delete(), 5)
