...
BOT_USERNAME = "Giveawaysedbot"  # Main bot username
BOT_TOKEN = "8251256866:AAFMgG9Csq-7avh7IaTJeK61G3CN3c21v1Y"  # Main bot token
...
# NEW: Game mode storage
game_mode = "paid"  # Default mode is "paid"
...
class Game:
    def __init__(self, user_id, username, game_type, chat_id):
        ...
        self.is_free_game = False  # Track if this is a free game
...
def save_data():
    try:
        data = {
            ...
            'game_mode': game_mode,  # Save game mode
...
def load_data():
    global allowed_groups, user_balances, user_profiles, user_game_history, total_ton_collected, game_mode
    try:
        ...
        game_mode = data.get('game_mode', 'paid')  # Load game mode
        ...
        logger.info(f"Data loaded: {len(allowed_groups)} groups, {len(user_balances)} users, mode: {game_mode}")
...
# ========== NEW COMMANDS: FREE AND PAID MODE ==========

@client.on(events.NewMessage(pattern='/free', incoming=True))
async def free_mode(event):
    ...
    try:
        ...
        await event.reply(
            "🆓 **FREE MODE ACTIVATED!**\n\n"
            "✅ All users can now play without deposits\n"
            "✅ No stars required\n"
            "✅ All games are free\n\n"
            "💡 Use `/paid` to switch back to paid mode"
        )
        ...
        for chat_id in allowed_groups:
            try:
                await client.send_message(
                    chat_id,
                    "🆓 **FREE MODE ACTIVATED!**\n\n"
                    "🎉 Good news! All games are now FREE!\n"
                    "🎮 Play without deposits\n"
                    "🎲 Start playing with `/play`"
                )
            except:
                pass
        
        logger.info("Game mode switched to FREE")
    except Exception as e:
        logger.error(f"Free mode error: {e}")
        await event.reply(f"❌ Error: {str(e)}")

@client.on(events.NewMessage(pattern='/paid', incoming=True))
async def paid_mode(event):
    if not event.is_private:
        return
    ...
    try:
        ...
        await event.reply(
            "💳 **PAID MODE ACTIVATED!**\n\n"
            "✅ Users need stars to play\n"
            "✅ Deposits required via `/deposit`\n"
            "✅ Normal game mode restored\n\n"
            "💡 Use `/free` to switch to free mode"
        )
        ...
        for chat_id in allowed_groups:
            try:
                await client.send_message(
                    chat_id,
                    "💳 **PAID MODE ACTIVATED!**\n\n"
                    "ℹ️ Games now require stars to play\n"
        ...
        logger.info("Game mode switched to PAID")
    except Exception as e:
        logger.error(f"Paid mode error: {e}")
        await event.reply(f"❌ Error: {str(e)}")

@client.on(events.NewMessage(pattern='/mode', incoming=True))
async def check_mode(event):
    if event.is_group and event.chat_id not in allowed_groups:
        return
    
    try:
        mode_icon = "🆓" if game_mode == "free" else "💳"
        mode_status = "FREE" if game_mode == "free" else "PAID"
        
        msg = f"{mode_icon} **Current Mode: {mode_status}**\n\n"
        
        if game_mode == "free":
            msg += "✅ All games are FREE\n✅ No deposits required\n✅ Play unlimited games"
        else:
            msg += "💰 Games require stars\n💳 Use `/deposit` to add balance\n💵 Check balance with `/bal`"
        
        await event.reply(msg)
    except Exception as e:
        logger.error(f"Mode check error: {e}")
        await event.reply("❌ Error checking mode")
...
async def add_group(event):
    ...
    try:
        ...
        try:
            ...
            mode_icon = "🆓" if game_mode == "free" else "💳"
            mode_text = "FREE - No deposits needed!" if game_mode == "free" else "PAID - Deposits required"
            
            await client.send_message(
                chat_id,
                f"🎮 **Game Bot Activated!**\n\n"
                f"{mode_icon} **Mode: {mode_text}**\n\n"
                ...
                "/deposit /depo  → 💳 Add stars (if paid mode)\n"
...
async def game_menu(event):
    ...
    try:
        ...
        mode_icon = "🆓" if game_mode == "free" else "💳"
        mode_text = "FREE MODE - Play without deposits!" if game_mode == "free" else f"PAID MODE - Balance: **{balance:,.0f} ⭐** (${balance_usd:.2f})"
        ...
        if game_mode == "free":
            menu_msg += (
                f"2. Enter any bet amount (virtual)\n"
                f"3. Select number of rounds\n"
                f"4. Choose throws per round\n"
                f"5. Play and have fun!\n\n"
                f"🆓 Free mode - No real stars needed!"
            )
        else:
            menu_msg += (
...
async def balance_command(event):
    ...
    try:
        ...
        msg = f"💰 **Balance for {username}**\n\n"
        
        if game_mode == "free":
            msg += f"{mode_icon} **FREE MODE ACTIVE**\n\nNo balance needed to play!\nEnjoy unlimited free games! 🎉"
        else:
            msg += f"⭐ Stars: **{balance:,.0f}**\n💵 USD: **${balance_usd:.2f}**"
...
async def tip_command(event):
    ...
    # Tips not available in free mode
    if game_mode == "free":
        await event.reply(
            "🆓 **Tips not available in FREE mode**\n\n"
            "Tips are only available in PAID mode."
        )
        return
...
async def profile_command(event):
    ...
    try:
        ...
        mode_icon = "🆓" if game_mode == "free" else "💳"
        balance_text = "FREE MODE" if game_mode == "free" else f"{balance:,.0f} ⭐"
        
        await event.reply(
            f"📢 **Profile**\n\n"
            f"ℹ️ User ID: `{user_id}`\n"
            f"⬆️ Rank: {rank_display}\n"
            f"{mode_icon} Balance: {balance_text}\n\n"
...
async def start_game(event, game_type):
    ...
    try:
        async with game_locks[user_id]:
            ...
            # Check balance only in paid mode
            if game_mode == "paid" and balance < 1:
                await event.reply(
            ...
            # Mark as free game if in free mode
            if game_mode == "free":
                game.is_free_game = True
            
            user_games[user_id] = game
            
            mode_msg = ""
            if game_mode == "free":
                mode_msg = "\n\n🆓 **FREE MODE** - Play without real stars!"
                bet_range = "1-10000"
            else:
                mode_msg = f"\n\n💰 Your balance: **{balance:,.0f} ⭐**"
                bet_range = f"1-{balance}"
            
            await event.reply(
                f"{game_info['icon']} **{game_info['name']} Game**\n\n"
                f"👤 Player: {username}"
                f"{mode_msg}\n\n"
                f"💵 Please send your bet amount ({bet_range})\n\n"
                f"Example: `100`"
            )
    except Exception as e:
...
async def handle_game_input(event):
    ...
    try:
        ...
        if game.state == GameState.WAITING_BET:
            # Different validation for free vs paid mode
            if game.is_free_game:
                # In free mode, allow any reasonable bet
                if value < 1:
                    await event.reply("❌ Minimum bet is 1 ⭐")
                    return
                if value > 10000:
                    await event.reply("❌ Maximum bet in free mode is 10000 ⭐")
                    return
            else:
                # In paid mode, check actual balance
                balance = user_balances.get(user_id, 0)
            ...
            # Only deduct balance in paid mode
            if not game.is_free_game:
            ...
            game_mode_icon = "🆓" if game.is_free_game else "💳"
            
            await event.reply(
                f"✅ **Game started** {game_mode_icon}\n\n"
...
async def handle_dice(event):
    ...
    try:
        ...
        # FIXED: Calculate round points correctly
        round_user_points = 0
        round_bot_points = 0
        ...
        if game.current_round < game.total_rounds:
            ...
            final_user_points = round_user_points
            final_bot_points = round_bot_points
            
            if game.user_score > game.bot_score:
                winnings = game.bet_amount * 2
                
                # Only add balance in paid mode
                if not game.is_free_game:
            ...
            balance_text = f"\n💰 New balance: **{balance:,.0f} ⭐**" if not game.is_free_game else ""
...
async def cancel_game(event):
    ...
    try:
        if user_id in user_games:
            ...
            # Only refund in paid mode
            if game.state == GameState.PLAYING and not game.is_free_game:
                user_balances[user_id] += game.bet_amount
...
async def help_command(event):
    ...
    try:
        mode_icon = "🆓" if game_mode == "free" else "💳"
        mode_status = "FREE - Play without deposits!" if game_mode == "free" else "PAID - Stars required"
        
        help_msg = (
            f"🎮 **Game Bot Help**\n\n"
            f"{mode_icon} **Mode: {mode_status}**\n\n"
            "**Game Menu:**\n"
        ...
        if game_mode == "paid":
            ...
            help_msg += "💰 `/bal` - Check mode status\n"
        ...
        if game_mode == "free":
            help_msg += (
                "1. `/dice` - Start game\n"
                "2. Enter bet amount (virtual)\n"
                "3. Choose rounds (1-3)\n"
                "4. Choose throws (1-3)\n"
                "5. Send dice emoji 🎲\n"
                "6. Bot plays automatically!\n"
                "7. Enjoy free gaming!"
            )
        else:
            help_msg += (
        ...
        if is_admin(event.sender_id):
            help_msg += (
                "\n\n**👑 Admin Commands:**\n"
                "🆓 `/free` - Enable free mode\n"
                "💳 `/paid` - Enable paid mode\n"
...
async def rain_command(event):
    ...
    # Rain not available in free mode
    if game_mode == "free":
        await event.reply(
            "🆓 **Rain not available in FREE mode**\n\n"
            "Rain is only available in PAID mode."
        )
        return
