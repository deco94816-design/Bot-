...
user_data = defaultdict(dict) # For bonus cooldowns

STARS_TO_USD = 0.0179
STARS_TO_TON = 0.01201014
MIN_WITHDRAWAL = 100  # Minimum withdrawal amount
BONUS_AMOUNT = 20 # New
BONUS_COOLDOWN_HOURS = 24 # New
...
class Game:
    def __init__(self, user_id, username, bet_amount, rounds, throw_count, game_type):
        ...
        self.round_started = False # New state for tracking round start
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id):
        help_text += (
            ...
            "/hb [amount] - Set House Balance\n"
        )
    
    await update.message.reply_html(help_text)

async def balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Modified to include Deposit/Withdraw buttons
    user_id = update.effective_user.id
    balance = get_user_balance(user_id)
    balance_usd = balance * STARS_TO_USD
...
async def withdraw_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Modified to enforce DM-only
    user_id = update.effective_user.id
    
    if update.effective_chat.type != update.effective_chat.PRIVATE:
        await update.message.reply_html("❌ <b>This command is only accessible in private messages (DM) with the bot.</b>")
        return
...
async def house_balance_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and len(context.args) > 0:
        try:
            new_hb = float(context.args[0])
            if new_hb < 0:
                raise ValueError("Cannot be negative")
            HOUSE_BALANCE_TON = new_hb
            await update.message.reply_html(
                f"✅ <b>House Balance Updated!</b>\n\n"
                f"🏦 New House Balance: <b>{HOUSE_BALANCE_TON:,.2f} TON</b>"
            )
        except ValueError:
            await update.message.reply_html("❌ Invalid amount! Please enter a valid number.")
            return
    else:
        await update.message.reply_html(
            f"🏦 <b>Current House Balance:</b> <b>{HOUSE_BALANCE_TON:,.2f} TON</b>\n\n"
            f"Usage (Admin only): /hb <amount>\n"
            f"Example: /hb 5000.50"
        )
...
async def bonus_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    last_bonus_time = user_data[user_id].get('last_bonus')
    
    if last_bonus_time and (now - last_bonus_time) < timedelta(hours=BONUS_COOLDOWN_HOURS):
        next_claim_time = last_bonus_time + timedelta(hours=BONUS_COOLDOWN_HOURS)
        ...
        await update.message.reply_html(
            ...
            f"You can claim your next bonus in: <b>{hours} hours and {minutes} minutes.</b>"
        )
        return
    ...
    await update.message.reply_html(
        f"🎁 <b>BONUS GRANTED!</b>\n\n"
        f"You received <b>{BONUS_AMOUNT} ⭐</b> as a daily bonus!\n\n"
        f"💰 Your new balance: <b>{balance:,} ⭐</b>"
    )

# --- GAME HANDLERS (No significant changes besides main logic fix) ---

async def start_game_command(update: Update, context: ContextTypes.DEFAULT_TYPE, game_type: str):
    # ... (No changes needed here for the bot roll fix, as this sets up the game)
    user_id = update.effective_user.id
    ...
    async with game_locks[user_id]:
        ...
        # Check for bet amount arguments
        bet_amount = None
        if context.args and len(context.args) > 0:
            ...
            # Store bet amount and proceed to rounds selection
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "start_withdraw":
            # Added check for DM here too for consistency with /withdraw
            if query.message.chat.type != query.message.chat.PRIVATE:
                 await query.edit_message_text("❌ <b>This command is only accessible in private messages (DM) with the bot.</b>\n\n"
                                             "Please use this button in a DM with me.", parse_mode=ParseMode.HTML)
                 return
        ...
        if data.startswith("throws_"):
            ...
            # NEW: Show option for bot to roll first
        ...
        # NEW: Handle bot first/user first selection - Modified to start round 1 correctly
        if data.startswith("start_game_"):
            parts = data.split("_")
            game_type = parts[2]
            who_first = parts[3]  # 'bot' or 'user'
            ...
            game.current_round = 1 # Start at round 1
            ...
            if game.bot_first:
                ...
                await query.edit_message_text(
                    ...
                    f"🔄 Rounds: <b>{rounds}</b>\n"
                    f"🎯 Throws per round: <b>{throws}</b>\n\n"
                    f"🤖 Bot is rolling first for Round 1...",
                    parse_mode=ParseMode.HTML
                )
                ...
                bot_total = sum(game.bot_results)
                ...
                await query.edit_message_text(
                    ...
                    f"🔄 Rounds: <b>{rounds}</b>\n"
                    f"🎯 Throws per round: <b>{throws}</b>\n\n"
                    f"👤 You roll first for Round 1! Send {throws}x {game_info['emoji']}",
                    parse_mode=ParseMode.HTML
                )
            return
...
async def handle_game_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if user finished their throws for the current round
    if len(game.user_results) % game.throw_count == 0:
        
        round_index = game.current_round - 1 # 0-based index for lists
        round_start_index = round_index * game.throw_count
        
        # Check if the bot still needs to roll for this round
        if len(game.bot_results) < round_start_index + game.throw_count:
            # Bot rolls now (This block runs if it's user-first, OR if it's bot-first but the bot's
            # results from the initial roll in button_callback for round 1 haven't been processed yet,
            # which is incorrect. The fix in button_callback ensures round 1 bot rolls are pre-populated.
            # For subsequent rounds in bot-first mode, the bot rolls *before* the user's turn starts.)

            # The only case this should run is for a user-first game, OR if an error occurred.
            # In a working bot-first game, the bot_results are already populated for the current round.
            if not game.bot_first: 
                await asyncio.sleep(0.5)
                
                bot_results_current_round = []
                for _ in range(game.throw_count):
                    bot_msg = await message.reply_dice(emoji=emoji)
                    bot_results_current_round.append(bot_msg.dice.value)
                    await asyncio.sleep(0.3)
                
                game.bot_results.extend(bot_results_current_round)

        # Calculate results for the just-completed round
        # Slice the lists based on the round's start index and throw count
        user_round_results = game.user_results[round_start_index:round_start_index + game.throw_count]
        bot_round_results = game.bot_results[round_start_index:round_start_index + game.throw_count]
        ...
        if game.current_round >= game.total_rounds:
            ...
            await message.reply_html(
                f"<b>Final Round {game.current_round} Results:</b>\n\n"
        ...
        # Continue to the next round
            ...
            if game.bot_first:
                ...
                await message.reply_html(
                    f"<b>Round {game.current_round-1} Results:</b>\n\n"
                    f"👤 Your total: <b>{user_round_total}</b>\n"
                    f"🤖 Bot total: <b>{bot_round_total}</b>\n\n"
                    f"{round_result}\n\n"
                    f"📊 Score: You <b>{game.user_score}</b> - <b>{game.bot_score}</b> Bot\n\n"
                    f"🤖 Bot is rolling for Round {game.current_round}..."
                )
                ...
                game.bot_results.extend(bot_results_current_round)
                bot_total = sum(bot_results_current_round)
                
                await asyncio.sleep(1)
                await message.reply_html(
                    f"🤖 <b>Bot's Round {game.current_round} total: {bot_total}</b>\n\n"
                ...
                await message.reply_html(
                    ...
                    f"{round_result}\n\n"
                    f"📊 Score: You <b>{game.user_score}</b> - <b>{game.bot_score}</b> Bot\n\n"
                    f"👤 Send {game.throw_count}x {emoji} for Round {game.current_round}!"
                )
...
def main():
    ...
    application.add_handler(CommandHandler(["balance", "bal"], balance_command)) # Alias added
    application.add_handler(CommandHandler(["deposit", "depo"], deposit_command)) # Alias added
    application.add_handler(CommandHandler("withdraw", withdraw_command))
    application.add_handler(CommandHandler("custom", custom_deposit))
    application.add_handler(CommandHandler("play", play_command))
    application.add_handler(CommandHandler("profile", profile_command))
    application.add_handler(CommandHandler("history", history_command))
    application.add_handler(CommandHandler("bonus", bonus_command)) # New Bonus Command
    ...
    application.add_handler(CommandHandler("hb", house_balance_command)) # New House Balance Command
