...
async def send_to_all_groups(client, message, interval, task_id):
    ...
    cycle_count = 0  # Track broadcast cycles for dots
    ...
    while True:
        try:
            ...
            # Add hidden dots using spoiler (increases each cycle)
            dots = "." * cycle_count
            message_with_dots = f"{message}\n||{dots}||"
            
            for group in groups:
                try:
                    sent_msg = await client.send_message(group.id, message_with_dots)
            ...
            log_message(client.phone, f"📊 Broadcast cycle #{cycle_count} complete - Success: {success_count}, Errors: {error_count}")
            log_message(client.phone, f"⏳ Waiting {interval} minutes before next broadcast (next will have {cycle_count + 1} dots)...")
