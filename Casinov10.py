...
class Game:
    def __init__(self, user_id, username, bet_amount, rounds, throw_count, game_type):
        ...
        self.bot_results = []
        self.is_demo = False
        self.bot_first = False  # NEW: Track if bot rolls first
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # NEW: Handle bot first/user first selection
...
async def handle_game_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(game.user_results) % game.throw_count == 0:
        ...
        # Only make bot rolls if bot hasn't rolled yet this round
        if not game.bot_first or len(game.bot_results) == len(game.user_results):
            bot_results = []
