    ...
    try:
        ...
        if data.startswith("throws_"):
            async with game_locks[user_id]:
                ...
                # Delete any existing game first to prevent score carryover
                if user_id in user_games:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if withdraw_state == 'waiting_amount':
        try:
            ...
            logger.error(f"Withdraw amount error: {e}")
            await update.message.reply_html("❌ An error occurred. Please try again.")
            return
        ...
        try:
            ...
            logger.error(f"Withdraw address error: {e}")
            await update.message.reply_html("❌ An error occurred. Please try again.")
            return
    ...
    if user_id in user_games:
        try:
                ...
                if len(user_throws) >= game.throw_count:
                    ...
                    if game.current_round >= game.total_rounds:
                        ...
                        if game.user_score > game.bot_score:
                            ...
                            final_message = (
                                f"🎉 <b>YOU WIN!</b>{demo_text}\n\n"
                                f"🎮 Game: {game_info['name']}\n"
                                f"💰 Bet: {game.bet_amount} ⭐\n"
                                f"🎯 Rounds: {game.total_rounds}\n\n"
                                f"📊 Final Score:\n"
                                f"{user_mention}: {game.user_score}\n"
                                f"🤖 Lenrao Game: {game.bot_score}\n\n"
                            ...
                            final_message = (
                                ...
                                f"{user_mention}: {game.user_score}\n"
                                f"🤖 Lenrao Game: {game.bot_score}\n\n"
                                f"🏆 Winner: 🤖 Lenrao Game\n"
                                f"💎 Lost: -{game.bet_amount} ⭐\n\n"
                                f"💰 New balance: <b>{balance} ⭐</b>"
                            )
                        else:
                            if not game.is_demo:
            ...
            logger.error(f"Game handling error: {e}")
            await update.message.reply_html("❌ An error occurred during the game. Please try again.")
...
async def successful_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_html("❌ Payment processed but an error occurred. Please contact support.")
